...
/* --------------------------- リストの横幅 ---------------------------
   標準は280px（CSS側）。右端のつまみを掴んで広げられる。
   幅は端末ごとの見た目の好みなので localStorage に持つ（サーバーには送らない）。 */
const LIST_W_KEY = 'listWidths';
const LIST_W_DEFAULT = 280;
const LIST_W_MIN = 200;
const LIST_W_MAX = 900;

function loadListWidths() {
  try { return JSON.parse(localStorage.getItem(LIST_W_KEY) || '{}') || {}; }
  catch (e) { return {}; }
}
function saveListWidths(w) {
  try { localStorage.setItem(LIST_W_KEY, JSON.stringify(w)); } catch (e) {}
}
function listWidthOf(listId) {
  const w = Number(loadListWidths()[listId]);
  return (w >= LIST_W_MIN && w <= LIST_W_MAX) ? w : 0;   // 0 = 標準のまま
}
function applyListWidth(el2, px) {
  // flex-basis も一緒に変えないと、flex: 0 0 280px の指定に負ける
  el2.style.width = px ? (px + 'px') : '';
  el2.style.flexBasis = px ? (px + 'px') : '';
}
function setListWidth(listId, px) {
  const all = loadListWidths();
  if (px) all[listId] = px; else delete all[listId];
  saveListWidths(all);
  updateWidthResetBtn();
}
// 全リストを標準幅へ戻す
function resetListWidths() {
  saveListWidths({});
  const els = document.querySelectorAll('#board .list');
  for (let i = 0; i < els.length; i++) applyListWidth(els[i], 0);
  updateWidthResetBtn();
  setStatus('リストの幅を戻しました');
}
// 幅を変えているリストが1つも無いときはボタンを隠す
function updateWidthResetBtn() {
  const btn = document.getElementById('widthResetBtn');
  if (!btn) return;
  const n = Object.keys(loadListWidths()).length;
  btn.classList.toggle('hidden', n === 0);
  btn.textContent = '↔ 幅を戻す（' + n + '）';
}

// リストの右端に、掴んで幅を変えるつまみを付ける
function attachResizer(wrap, list) {
  const bar = el('div', 'list-resizer');
  bar.title = '左右にドラッグして幅を変える（ダブルクリックで標準に戻す）';
  bar.addEventListener('dblclick', function (e) {
    e.stopPropagation();
    applyListWidth(wrap, 0); setListWidth(list.id, 0);
  });
  const start = function (clientX) {
    const w0 = wrap.getBoundingClientRect().width;
    const x0 = clientX;
    document.body.classList.add('resizing-list');
    const move = function (x) {
    ...
    let last = w0;
    const onMouseMove = function (ev) { last = move(ev.clientX); };
    const onTouchMove = function (ev) {
      if (ev.touches && ev.touches[0]) { last = move(ev.touches[0].clientX); ev.preventDefault(); }
    };
    const end = function () {
      ...
      setListWidth(list.id, Math.abs(last - LIST_W_DEFAULT) < 4 ? 0 : last);
    };
  ...
  bar.addEventListener('mousedown', function (e) { e.preventDefault(); e.stopPropagation(); start(e.clientX); });
...
let relBoardRect = null;   // 盤面の矩形。線をはみ出させないために relAnchor から見る
let relRaf = 0;
...
function relAnchor(a, b) {
  ...
  if (b.l > a.r + 8) {                     // 相手が右にいる
    ...
    const vgap = down ? (b.t - a.b) : (a.t - b.b);
    if (vgap < 60) {
      // すぐ隣のカード。まっすぐ結べばよい
      ...
      // 離れた2枚。まっすぐ下ろすと間のカードを縦に貫いて「変な線」に見えるので、
      // リストの外（リストとリストの間の余白）へ逃がして回り込ませる。
      const br = relBoardRect || { left: 0, right: window.innerWidth };
      const listL = Math.min(a.ll, b.ll), listR = Math.max(a.lr, b.lr);
      const goRight = (listR + 26) < (br.right - 8);   // 右に余白が無ければ左へ回す
      const lane = goRight ? Math.min(listR + 26, br.right - 10)
                           : Math.max(listL - 26, br.left + 10);
      x1 = goRight ? a.r : a.l; y1 = a.cy;
      x2 = goRight ? b.r : b.l; y2 = b.cy;
      // 制御点をこの位置に置くと、曲線のいちばん外side が lane を通る
      const mid = (x1 + x2) / 2;
      const ctrl = (4 * lane - mid) / 3;
      c1x = ctrl; c1y = y1; c2x = ctrl; c2y = y2;
    }
  }
...
function relDraw() {
  ...
  Array.prototype.forEach.call(board.querySelectorAll('.card'), function (n) {
    ...
    // ll/lr = このカードが入っているリストの左右。同じリスト内の回り込みで使う
    const lrect = (n.closest('.list') || n).getBoundingClientRect();
    pos[id] = { l: r.left, r: r.right, t: t, b: b, cx: (r.left + r.right) / 2, cy: (t + b) / 2,
                ll: lrect.left, lr: lrect.right, el: n };
  });
...
/* ---------------------- つなぐモード（カードを順にクリック） ----------------------
   カード詳細を開いて検索するより速い道。上部バーの「✚ つなぐ」か、
   カードの Ctrl(⌘)+クリックで始まる。あとは相手をクリックするだけ。
   flow は「つないだ先が次の起点」になるので A→B→C と鎖で引ける。
   part / rel は起点を固定したままなので、1枚に何枚でもぶら下げられる。 */

let relLinkOn = false;
let relLinkFrom = null;
let relLinkType = 'flow';
let relLinkWasVisible = false;

function relLinkApply() {
  document.body.classList.toggle('rel-linking', relLinkOn);
  document.querySelectorAll('.card.rel-origin').forEach(function (n) { n.classList.remove('rel-origin'); });
  if (relLinkOn && relLinkFrom) {
    const n = cardNode(relLinkFrom);
    if (n) n.classList.add('rel-origin');
  }
  const b = $('#relLinkBtn');
  if (b) b.classList.toggle('on', relLinkOn);
  renderRelLinkBar();
}

function renderRelLinkBar() {
  const bar = $('#relLinkBar');
  if (!bar) return;
  bar.classList.toggle('hidden', !relLinkOn);
  if (!relLinkOn) return;
  const sel = $('#relLinkType');
  if (sel) sel.value = relLinkType;
  const msg = $('#relLinkMsg');
  const from = relLinkFrom ? relLive(relLinkFrom) : null;
  if (!from) {
    msg.innerHTML = '<b>つなぐ元</b>のカードをクリックしてください';
  } else {
    msg.innerHTML = '<b>' + esc(from.title) + '</b> ' + REL_TYPES[relLinkType].mark
      + ' <span class="rel-linkbar-hint">'
      + (relLinkType === 'flow' ? '相手をクリック（続けて鎖のようにつなげます）'
                                : '相手をクリック（この1枚に何枚でも足せます）')
      + '</span>';
  }
}

function relLinkStart(fromId) {
  if (!relLinkOn) {
    relLinkOn = true;
    // つないだ結果がその場で見えるように、線の表示を自動で入れる
    relLinkWasVisible = relsVisible;
    if (!relsVisible) { relsVisible = true; updateRelBtn(); }
  }
  relLinkFrom = fromId || null;
  relRerender();
  relLinkApply();
  setStatus(fromId ? '' : 'つなぐ元のカードをクリックしてください');
}

function relLinkStop() {
  if (!relLinkOn) return;
  relLinkOn = false;
  relLinkFrom = null;
  if (!relLinkWasVisible && relsVisible) { relsVisible = false; updateRelBtn(); }
  relRerender();
  relLinkApply();
  setStatus('つなぐモードを終わりました');
}

function relLinkToggle() {
  if (relLinkOn) relLinkStop(); else relLinkStart(null);
}

function relLinkPick(card) {
  const fromId = relLinkFrom;
  const type = relLinkType;
  if (!fromId || fromId === card.id) return;
  relAdd(fromId, card.id, type).then(function (ok) {
    if (!ok) { relLinkApply(); return; }
    // 押し間違えても Ctrl+Z / バーの「直前を取り消す」で戻せる
    pushUndo('つながりを追加', function () { return relRemove(fromId, card.id, type); });
    // 流れは鎖で引けるように起点を進める。内訳・関連は起点を動かさない
    if (type === 'flow') relLinkFrom = card.id;
    relRerender();
    relLinkApply();
    if (openCardId) { const cur = currentCard(); if (cur) renderRels(cur); }
  });
}

// カードのクリックを、つなぐモードが先に受け取る。true を返したらカードは開かない
function relLinkClick(card, e) {
  if (e && (e.ctrlKey || e.metaKey)) {
    e.preventDefault();
    relLinkStart(card.id);       // モード外なら開始、モード中なら起点を差し替え
    return true;
  }
  if (!relLinkOn) return false;
  if (!relLinkFrom || relLinkFrom === card.id) {
    relLinkFrom = (relLinkFrom === card.id) ? null : card.id;
    relLinkApply();
    return true;
  }
  relLinkPick(card);
...
function renderList(list) {
  ...
  applyListWidth(wrap, listWidthOf(list.id));   // 前回広げた幅を復元
  attachResizer(wrap, list);
  return wrap;
}
...
function renderCard(card) {
  ...
  if (relLinkOn && relLinkFrom === card.id) c.classList.add('rel-origin');
  c.addEventListener('click', function (e) {
    if (relLinkClick(card, e)) return;   // つなぐモード中はカードを開かない
    openModal(card.id);
  });
  return c;
}
...
function bindUI() {
  ...
  $('#relLinkBtn').addEventListener('click', relLinkToggle);
  $('#relLinkDone').addEventListener('click', relLinkStop);
  $('#relLinkUndo').addEventListener('click', doUndo);
  $('#relLinkType').addEventListener('change', function () {
    relLinkType = relType(this.value);
    renderRelLinkBar();
  });
  ...
  document.addEventListener('keydown', function (e) {
    ...
    else if (!$('#relmap').classList.contains('hidden')) hideRelMap();
    else if (!$('#boardHome').classList.contains('hidden')) hideHome();
    else if (relLinkOn) relLinkStop();
  });
