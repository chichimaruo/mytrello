...
- **カード同士のつながり**（`Cards.rels`・v20）: カードを向き付きで結ぶ。**線を引くだけで終わらせない**のが要点
  - **つなぐモード**（上部バー「✚ つなぐ」／カードを **Ctrl(⌘)+クリック**）: 起点→相手とカードをクリックしていくだけでつながる。
    `flow` はつないだ先が次の起点になるので A→B→C と鎖で引ける。`part`/`rel` は起点が動かないので1枚に何枚でもぶら下がる。
    追加は毎回 `pushUndo` に積むので Ctrl+Z／バーの「↩ 直前を取り消す」で戻せる。Esc で終了
  - カード詳細の「🔗 つながり」欄で相手を検索してつなぐ／外す（画面に出ていない相手はこちら）。輪（A→B→A）になる向きは拒否する
  - 上部バー「🔗 つながり」で**盤面に線を重ねる**（ONの間だけリスト間隔を56pxに広げて線の通り道を作る）。カードに触れると、その線とつながっている相手だけが浮き上がり（青枠＋影）、**関係のないカードは沈む**（`.rel-cold`＝薄く＋彩度落とし）。つながりの無いカードに触れたときは何も沈めない（盤面がちらつくため）
...
>
> - **2026-08-09 時点の実態＝`backupNow` の1個だけ**。これで**正常**。
>   `sendDueReminders`（期限リマインダーのメール）と `runRecurring`（繰り返しカード）が未登録なのは
>   **2026-08-09 に「今は使わない」と判断した結果**であって、不具合ではない。直そうとしなくてよい。
>   使いたくなったらアプリの ⚙設定 からオンにするだけ（実装はどちらも入っている）。
...
## 10.5 ★不調のときは、まずここを見る（2026-09-16 追加）

**症状から原因へ最短で辿るための表。心当たりがあれば、推測より先に `inspectCardsRaw` を叩くこと。**

| 症状 | 真っ先に疑うこと |
|---|---|
| 保存したのに値が入らない。エラーは出ず、APIは `true` を返す | **Cardsシートの列ずれ**（下記）。まず `inspectCardsRaw` |
| 完了チェックを付けても、開き直すと外れている | 同上。`done` の書き込みが隣の列へ逃げている |
| カスタムフィールドの値が消える／`ratings` 列に入っている | 同上 |
| カードのリンクが `{}` になっている、カバーが化けた | 同上（`links` に `sync` の値が入る等） |
| アプリ版だけ `unknown function` | `API_ALLOWED` への登録漏れ。関数を足したら必ず追加（§2-A） |
| 夜間バッチだけ古い動きのまま | トリガーは「導入」列のバージョンで動く。`clasp push` だけでは変わらない（§8） |

### 2分でできる健全性チェック
```
cd "H:\マイドライブ8 アプリ開発\My Trello Project"
python -c "import sys;sys.path.insert(0,'.');from _mt import call;d=call('inspectCardsRaw',[1,0]);print(d['lastCol'],d['header']==d['schema'])"
```
**`28 True` 以外が出たら列がずれている。** `repairCardsColumns()` を実行すれば直る（§11-18）。

---

## 10.6 コマンドからデータを読み書きする（`_mt.py`）

チャットや調査からアプリのデータを直接触るための窓口。**アプリのJSON APIを叩くだけ**なので、
`API_ALLOWED` に載っている関数は全部使える。

```
python _mt.py getMeta                       # ボード/リスト/ラベル/フィールド一覧
python _mt.py getCards "[\"<boardId>\"]"      # そのボードのカード
```
```python
import sys; sys.path.insert(0, '.')
from _mt import call
call('updateCard', [cardId, {'fields': {...}}])
```

- 接続情報は **`%USERPROFILE%\.mytrello\config.json`**（`url` と `token`）。
  **プロジェクトの外**に置いてあるので、公開リポジトリにも会話ログにも出ない。
- **転送に PowerShell の `Invoke-WebRequest` を使っている。** Python の `urllib` は Apps Script の
  リダイレクトで 404、`curl` は 411（Length Required）になった。ここは変えないこと。
- 応答が **JSON ではなく HTML で返ってくることがある**（Apps Scriptの気まぐれ）。`_parse` が
  HTMLからJSONを取り出すので、そのまま使える。
- PowerShell の `ConvertFrom-Json` は**大きな応答を扱えない**（`getAllCards` は約170KB＝解析不能）。
  調査は必ず Python 側でやること。

---

## 10.7 Trello から消えたカスタムフィールドの値を取り戻す（2026-09-16 実施）

有料プランをやめると Custom Fields の Power-Up が無効になり、値が**見えなくなる**。
ただし**消えてはいない**（再課金すると復活するのが証拠）。無料のままでも次の方法で取り出せた。

1. **ボードのJSONエクスポートは無料プランでも使える。**
   Trelloでボードを開く → メニュー → さらに表示 → 印刷とエクスポート → JSONとしてエクスポート
   （または `https://trello.com/b/<ボードID>.json` を直接開く）
2. ただし **`customFields` と `customFieldItems` は空**で出てくる（Power-Upが無効なため）。
   **本命は `actions`（操作履歴）**。`updateCustomFieldItem` に、入力した値がそのまま残っている。
3. カード名×項目ごとに**最も新しい action** を採用すれば、当時の値を再現できる。
   ドロップダウンの値は `idValue`（選択肢ID）で入っているので、
   `createCustomField` / `updateCustomField` の `options` から ID→表示名 の対応表を作って引く。
4. **エクスポートは actions を直近1000件しか含まない。** 古い時期に作った項目は対応表が手に入らない。
   実際「評価」の選択肢名は取れず、**★いくつかは本人に判定してもらった**（IDの生成時刻＝作成順は手がかりになる）。
5. **時期の違うエクスポートが複数あれば統合する**と、履歴の窓が広がって復元できる件数が増える。

> 取り込み先は `_mt.py` の `updateCard` で、カード名の**完全一致**で突き合わせる。
> 全角/半角スペース違いの同名カードが実在したので、正規化して突き合わせると取り違える。
...
18. **★★見出しの無い列が1本混ざると、書き込みが全部ずれる（2026-09-16に実際に発生）**

    **これが §4 の不変条件が現実に破られた唯一の事例。同じ症状が出たら、まず §10.5 を見ること。**

    - **何が起きたか**: `Cards` シートに **見出しが空の列**（16列目）と **重複した `embHash`**（30列目）が
      紛れ込んでいた。`Code.gs` は**書き込みを列番号（SCHEMAの順）で、読み取りを見出し名で**行うため、
      `allDay` より後ろの書き込みが**すべて1つ左の列**に入っていた。
    - **たちが悪い点**: エラーが出ない。`updateCard` は `true` を返す。**保存したのに値が入らない**だけ。
      `title` `desc` `due` `start` など15列目までは正常に動くので、余計に気づきにくい。
    - **実害**: 完了チェック51枚が保存されず（アプリ上は常に未完了）、カスタムフィールド42枚が `ratings` 列へ、
      カバー53枚が `fields` 列へ。さらに列が増えた後に**新規作成された10枚は行ごとずれ**、読み取りも狂っていた。
    - **直し方**: `repairCardsColumns()` を実行する。見出しが空／SCHEMAに無い／重複した列を右から削除するだけ。
      **列を消すと見出しとデータが一緒に左へ詰まるので、正常だった行は何もせず正しくなる**（186枚がこれで復旧）。
      そのあと、迷い込んだ値を正しい列へ書き戻す（このときの控えは `inspectCardsRaw` の生データから作る）。
...
- **同じリストの中で離れた2枚**: 素直に上下を結ぶと、線が間のカードを縦に貫いて
  「関係ないカードに線が刺さっている」ように見える（2026-09-17 に指摘を受けて修正）。
  縦の空きが60px未満＝隣どうしのときだけ直線で結び、それ以上離れていたら
  **リストの外（リストとリストの間の余白）へ逃がして回り込ませる**。
  逃がす側は盤面の右に余白があれば右、無ければ左。通したい x（lane）に対して
  制御点を `(4*lane - mid) / 3` に置くと、曲線のいちばん外側がちょうど lane を通る。
  そのためにカードごとに、属するリストの左右 `ll`/`lr` も `pos` に持たせている。
...
**つなぐモード（`relLinkOn` / `relLinkFrom` / `relLinkType`）**
- カードの click ハンドラの先頭で `relLinkClick(card, e)` が横取りする。**true を返したらカードを開かない**。
- モード中は `body.rel-linking` が付き、CSSで `.card-edit-btn` を隠し `.card-check` の `pointer-events` を切る。
  **カードのどこを押しても「つなぐ」になる**ようにするため（小さなボタンに当たって別の動作をすると混乱する）。
- 起点の枠 `.card.rel-origin` は `renderCard` でも付け直す（`relRerender()` でDOMが作り直されるため）。
- モードに入ると線の表示を自動でONにし、抜けるときに**入る前の状態へ戻す**（`relLinkWasVisible`）。
...
- **2026-09-16（保守）**：Cardsシートの列ずれを発見・修復（§11-18）。診断用 `inspectCardsRaw` と修復用 `repairCardsColumns` を追加。
  併せて Trello から評価などを復元し、飲み物備忘録を★順に並べ替え（§10.7）
